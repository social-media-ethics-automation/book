# Understanding the Mastodon Bot Code

Let's look more at that program that creates one Mastodon post.
...
mastodon = Mastodon(api_base_url = "https://mastodon.social", access_token = "<strong style="color:black;background-color:lightgreen">m#5@_fake_access_token_$%Ds</strong>")

mastodon.toot("<strong style="color:black;background-color:lightgreen">This post was made by a computer program!</strong>")
</pre>

The first highlighted piece of code is for your "access token" (note: If you make your account at a different domain name than the default "mastodon.social," then you'd also need to change the part above that has `https://mastodon.social`). You can create a Mastodon account by [following the steps here](../../appendix/making_bot_account.md) (I've put a fake "access token" in the code for now):

The final highlighted piece of code is the text to post to Mastodon. You can change the text of these values to change what text to post.

So, by changing those sections of code, you run this program to post whatever you want to post on Mastodon. It is, of course, much easier to just open Mastodon and post something, but as we get to more complicated programs, we'll start to see more of the power (and pitfalls) of automation on social media.
...
mastodon = Mastodon(api_base_url = "https://mastodon.social", access_token = "m#5@_fake_access_token_$%Ds")
...
The purpose of this line of code that loads another set of code. The it loads is called [Mastodon.py](https://mastodonpy.readthedocs.io) which is code specially written to help make programs that work with Mastodon.
...
The purpose of this code is to connect to Mastodon and login with your "access token". Again, you'll have to replace our fake access token with your actual access token in the code if you want it to work with your account.
...
This is the line of code where a Mastodon post is actually made (called a "toot"). The new post will have the text that is inside the quotation marks.
